...
-- Add is_public column to menu_items
ALTER TABLE public.menu_items ADD COLUMN is_public boolean NOT NULL DEFAULT true;

-- Update public SELECT policy to also check is_public
DROP POLICY IF EXISTS "Public can view available menu items" ON public.menu_items;
CREATE POLICY "Public can view available menu items"
ON public.menu_items
FOR SELECT
USING ((auth.uid() IS NULL) AND (is_available = true) AND (is_public = true));

-- Update create_public_order to only allow public items
